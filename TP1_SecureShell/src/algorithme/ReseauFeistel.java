...
public class ReseauFeistel {
	...
	private static byte[] encrypt64Bloc(byte[] bloc,byte[][] subkeys, boolean isDecrypt) {
	    ...
	    L = Util.extractBits(tmp, 0, IP.length/2);
	    R = Util.extractBits(tmp, IP.length/2, IP.length/2);
	    ...
	    tmp = Util.concatBits(R, IP.length/2, L, IP.length/2);
	...
	private static byte[] permutFunc(byte[] input, int[] table) {
	     ...
	     for (int i = 0; i < table.length; i++) {
	          int val = Util.extractBit(input, table[i] - 1);
	          Util.setBit(out, i, val);
	     }
	     return out;
	}
	...
	private static byte[][] generateSubKeys(byte[] key) {
		...
		byte[] C = Util.extractBits(tmpK, 0, PC1.length/2);
		byte[] D = Util.extractBits(tmpK, PC1.length/2, PC1.length/2);

		for (int i = 0; i < 16; i++) {

			C = Util.rotLeft(C, 28, keyShift[i]);
			D = Util.rotLeft(D, 28, keyShift[i]);

			byte[] cd = Util.concatBits(C, 28, D, 28);
